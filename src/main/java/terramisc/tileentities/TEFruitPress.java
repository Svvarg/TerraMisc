...
public class TEFruitPress extends TEHopper
{
	...
	private void pressItems()
	{
		...
		if(item != null)
		{
			...
			{
				FruitPressRecipe recipe = FruitPressManager.getInstance().getMatchingRecipe(item.getItem());
				barrel.addLiquid(new FluidStack(recipe.getOutput(), recipe.getOutputAmount()));
			}
		}
	}
	...
	public int hasPressableItem()
	{
		...
		for(int i = 0; i < getSizeInventory(); i++)
		{
			if(getStackInSlot(i) != null && FruitPressManager.getInstance().getMatchingRecipe(getStackInSlot(i).getItem()) != null)
